...
class ExtenderTemplateImport implements ToCollection,WithEvents
{
    public $sheetNames;
    public $sheetData;
    ...
    */
    ...
    public function registerEvents() : array
    {
        return [
            ...
            AfterImport::class => function () {
                            ...
                            foreach ($this->sheetData[$sheetKey] as $key => $excelRow) { //循环这个Sheet表的每一行
                                if ($key >= $this->sheetStartRow) { //从开始行开始处理数据
                                    ...
                                    $this->resultData[$sheetKey][] = [
                                        ...
                                        'products'      => $this->getExtenderRowItems($excelRow)//获取每一行对应的产品真实名称
                                    ];
                                }
                            }
                ...
                Excel::store(
                    new ExtenderExport($this->data, $this->itemIncludeHarness, $this->splitStr),
                    "Extender-".date('Y-m-dHis', time()).".xlsx"   //导出的文件名
                );
     ...
     * @return array
     */
    public function getExtenderRowItems($excelRow)
    {
        $items = [];
        for ($i=$this->data['itemFirstKey']; $i<=$this->data['itemEndKey'];$i++ ){
            if($excelRow[$i]){ //如果这一行存在值
                 ...
                 * --------------------------------------
                 * Extender的item信息说明
                 * 例如 ：列名为 Pos - 2208inch/Neg - 1896inch
                 * 结果：Pos - 2208inch  Neg - 1896inch
                 * --------------------------------------
                 */
                $columnItemName = $this->data['itemList'][$i-$this->data['itemFirstKey']]; //当前列的item名称
                $columnItemNames = explode($this->splitStr,$columnItemName);
                foreach ($columnItemNames as $columnItemName){
                    $items[] = $columnItemName;
                }
                /**
                 * --------------------------------------
                 * 处理Extender的Item结束
